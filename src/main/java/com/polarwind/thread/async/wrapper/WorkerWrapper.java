package com.polarwind.thread.async.wrapper;
...
import com.polarwind.thread.async.callback.DefaultCallback;
import com.polarwind.thread.async.callback.ICallback;
import com.polarwind.thread.async.callback.IWorker;
import com.polarwind.thread.async.exception.SkippedException;
import com.polarwind.thread.async.executor.timer.SystemClock;
import com.polarwind.thread.async.worker.DependWrapper;
import com.polarwind.thread.async.worker.ResultState;
...
public class WorkerWrapper<T, V> {
  ...
  private WorkerWrapper(String id, IWorker<T, V> worker, T param, ICallback<T, V> callback, Boolean allowInterrupt) {
    if (worker == null) {
  ...
  private void beginNext(ExecutorService executorService, long now, long remainTime) {
    ...
    for (int i = 0; i < nextWrappers.size(); i++) {
      int finalI = i;
      futures[i] = CompletableFuture.runAsync(() -> nextWrappers.get(finalI).work(executorService, WorkerWrapper.this,
        remainTime - costTime, forParamUseWrappers), executorService);
    }
    try {
  ...
  public static class Builder<W, C> {
    ...
    public Builder<W, C> allowInterrupt(Boolean allowInterrupt) {
      this.allowInterrupt = allowInterrupt;
      return this;
    }
    ...
    public WorkerWrapper<W, C> build() {
      WorkerWrapper<W, C> wrapper = new WorkerWrapper<>(id, worker, param, callback, allowInterrupt);
