package com.polarwind.thread.async.executor;
...
import com.polarwind.thread.async.callback.DefaultGroupCallback;
import com.polarwind.thread.async.callback.IGroupCallback;
...
public class Async {
  ...
  private static final ThreadPoolExecutor COMMON_POOL =
    new ThreadPoolExecutor(Runtime.getRuntime().availableProcessors() * 2, 1024, 15L, TimeUnit.SECONDS,
  ...
  public static boolean beginWork(long timeout, ExecutorService executorService,
    ...
    for (int i = 0; i < workerWrappers.size(); i++) {
      ...
      futures[i] =
        CompletableFuture.runAsync(() -> wrapper.work(executorService, timeout, forParamUseWrappers), executorService);
    }
    try {
